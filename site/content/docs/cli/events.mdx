...
records every column transition and every workspace state change across
the hub. The orchestrator follows this log as its trigger stream — to
see what the orchestrator is seeing, run `events tail --follow` in
another pane.
