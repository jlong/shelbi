...
[orchestrator](/docs/concepts/orchestrator) consumes on bootstrap: five
sections — `## Board`, `## Workspaces`, `## Zen`, `## Handoff`, and
`## Daemon`. Both the plain summary and `--full` are idempotent and safe to
re-run.
...
| --- | --- | --- | --- |
| `--full` | flag | off | Emit the full sectioned bootstrap payload (five sections: Board, Workspaces, Zen, Handoff, Daemon). Idempotent — safe to re-run. |
