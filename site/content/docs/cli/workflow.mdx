...
Workflow files live at `<config-root>/workflows/<name>.yaml`, where
`<config-root>` is `~/.shelbi/projects/<project>/` in the default
[global mode](/docs/concepts/config-modes) and `<repo>/.shelbi/` in
in-repo mode. A file's basename is its name; the in-file `name:` field
must match. The loader auto-creates `default.yaml` on first load when
the directory is empty, so most projects never need to touch this
command — it's there when you want a second pipeline (a docs-only flow,
a research track that never opens PRs, a feature-stacking flow off a
long-lived branch).
