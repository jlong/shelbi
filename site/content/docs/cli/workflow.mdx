...
order: 23
summary: Manage the per-project workflow YAML files — list, show, scaffold, and edit the status schemas issues run under.
---
...
`shelbi workflow` manages the YAML files that declare the statuses an issue moves
through, who owns each one, and which git side-effects fire on each transition.
Every project has a built-in `default` workflow: the canonical six-status
flow (`Backlog → Todo → In Progress → Review → Done → Canceled`). These
subcommands surface it alongside any custom workflows the project authors.
...
Scaffold a new workflow YAML pre-populated with the canonical six-status
default (the five historical lanes plus `Canceled`). The file's `name:` field is set to `<NAME>` so it matches the
...
`_`. The name doubles as a YAML identifier referenced from issue
...
- [`shelbi issue`](/docs/cli/issue) — the `--workflow` flag on
  `issue add` and the workflow-aware `--to` validation on `issue move`.
