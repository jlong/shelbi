...
Two failure modes are transient (direct SSH stays healthy) but were noisy in the
events log, so the recheck now handles them explicitly:
...
- Loopback-port exhaustion on a TCP-fallback host is now surfaced distinctly as
  `detail=loopback_port_exhausted band=<lo>-<hi>` rather than the ambiguous old
  "no free loopback port in band" line, so it reads apart from a master-open
  blip. Before allocating, the recheck reclaims the port its own prior master
  was holding, then sweeps the configured band.
