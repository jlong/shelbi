...
order: 12
summary: "Manage projects: add a new one with the detected setup card, or migrate an existing global-mode project into committed in-repo mode."
---
...
directory, else `$SHELBI_ROOT`, else the install-time default),
`-p, --project <PROJECT>` (defaults to `$SHELBI_PROJECT` or the registered
project whose `work_dir` contains the current directory), and `-y, --yes`
(accept the detected plan without prompts — meaningful for `add`) flags. On
`migrate-to-in-repo` the project flag's value name renders as `<NAME>`.
