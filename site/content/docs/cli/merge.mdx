...
`shelbi merge` is the bridge between a task in `review` and a clean
default branch. By default it performs a local squash-merge of the
task's branch into the project's default branch (typically `main`)
using the workspace's commit message. Pass `--pr` to push the branch
and open a GitHub pull request instead — useful when your team's
workflow requires a code-review roundtrip before anything lands.
