...
```text
shelbi zen pr-create [OPTIONS] <ID>
```
...
Otherwise, it opens a new PR. Prints the PR number on stdout. Pass the probe's
`head_sha` with `--match-head-commit` to reject any branch movement between
review and PR creation.

| Argument | Type | Default | Description |
| --- | --- | --- | --- |
| `<ID>` | string | — | Task id whose branch to push and PR (required). |
| `--match-head-commit <SHA>` | string | none | Require the task branch to match this probe SHA before pushing. |
