...
The toggle subcommands (`on`, `off`, `pause`) flip the project's mode in
`state.json` and write a `<ts> project=<project> mode=zen <prev> -> <new>
reason=user:cli` line to `~/.shelbi/events.log` (the leading
`project=<project>` scope keeps one project's toggle from being read by
another project's orchestrator). `status` is read-only: it prints the current
mode and writes no event. The orchestrator reads those events to know when to
switch behavior.
...
| --- | --- | --- | --- |
| `<TASK_ID>` | string | — | Task id to probe (required). |

## pr-create

```text
shelbi zen pr-create <TASK_ID> --match-repository <REPOSITORY> --match-repository-id <REPOSITORY_ID> --match-base-branch <BRANCH> --match-base-commit <BASE_SHA> --match-integration-commit <INTEGRATION_SHA> --match-head-commit <HEAD_SHA> [--match-published-head-commit <SHA>]
```
...
The six `--match-*` flags above are required. Copy them from the immediately
...
A seventh flag, `--match-published-head-commit <SHA>`, is optional and only
`pr-create` reads it. Pass the probe report's `published_head_sha` when the
probe rebased the task branch onto a moved base — so `head_sha` is a
never-pushed local commit while the remote task branch still points at the
pre-rebase tip. It lets `pr-create` recognize that pre-rebase remote tip as the
flow's own prior state rather than a concurrent update. Omit it to preserve the
default acceptance rules. (`ci-watch` and `pr-merge` also accept the flag
because they share the same pinned-identity group, but neither acts on it.)

| Argument or flag | Type | Default | Description |
| --- | --- | --- | --- |
| `<TASK_ID>` | string | — | Task id whose branch to push and PR (required). |
...
| `--match-published-head-commit <SHA>` | string | — | Optional. Probe `published_head_sha` — the pre-rebase remote tip, recognized as the flow's own prior state when the probe rebased onto a moved base. |
