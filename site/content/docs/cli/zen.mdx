...
the durable task branch, and the report's `head_sha` identifies the exact
commit used for every check and fact.
...
```text
shelbi zen pr-create <ID> --match-head-commit <SHA>
```
...
`--match-head-commit` is required. Pass the `head_sha` from the immediately
preceding `probe` report. Shelbi rejects the operation if the durable task
branch moved after the probe, so an older or newer commit cannot silently
replace the commit whose checks and diff were reviewed.
...
Watch the PR number returned by the pinned `pr-create` call and retain that
probe's `head_sha`. The following `pr-merge` call requires the same SHA and
refuses to merge if the PR head moved while CI was running.
...
```text
shelbi zen pr-merge <PR_NUMBER> --match-head-commit <SHA>
```
...
`--match-head-commit` is required and must receive the same probe
`head_sha` passed to `pr-create`. Shelbi verifies the live PR head before
merging, so a branch update during CI stops the merge.

| Argument or flag | Type | Default | Description |
| --- | --- | --- | --- |
| `<PR_NUMBER>` | integer | — | The PR number to merge (required). |
| `--match-head-commit <SHA>` | string | required | Require the live PR head to equal the probe SHA before merging. |
...
PR=$(shelbi zen pr-create docs-write-cli-reference-pages --match-head-commit "$HEAD_SHA")
shelbi zen ci-watch "$PR" --timeout 10m && \
