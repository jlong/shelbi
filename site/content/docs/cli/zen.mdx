...
Shelbi validates the complete probe identity before the GitHub merge or queue
mutation and verifies the result afterward. It supplies `expectedHeadOid`,
which atomically leases the reviewed head only. GitHub exposes no atomic
expected-base or expected-repository input.

This leaves a precise residual limitation: a concurrent retarget with an
unchanged head between validation and mutation can land or enqueue into the
new base, and a post-response rejection is too late to undo it. The limitation
requires separate human design and this path must not be described as atomic
base pinning. Ordinary merge and queue behavior otherwise remains unchanged.
A landed PR prints its merge SHA or `sha-pending`; a queued PR prints `queued`
and remains in handoff for a later exact-provenance retry.
