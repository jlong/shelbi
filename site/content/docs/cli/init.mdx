...
root, branch, origin, runner, tmux, workspace count, and naming preset, then
creates the same project and Welcome card as Enter on the interactive setup
card.
...
`--runner` selects the runner for generated workspaces and is also the default
orchestrator runner. Use `--orchestrator-runner` only when those should differ.
The selected runners must be installed on `PATH`.
...
| `--runner <RUNNER>` | `claude` \| `codex` | the only detected runner | Runner for generated workspaces. Required when both supported runners are installed. |
...
| `--workspace-count <COUNT>` | integer, at least 1 | machine recommendation | Number of development workspaces. A separate review workspace is always added. `--workspaces` is an alias. |
| `--workspace-preset <PRESET>` | `phonetic` \| `greek` \| `toy-story` | detected default | Naming preset for generated workspaces. |
...
Without `-y`, `shelbi init` retains the explicit config-mode workflow:

| Flag | Type | Default | Description |
| --- | --- | --- | --- |
| `--mode <MODE>` | `in-repo` \| `global` | asked interactively | Choose committed in-repo config or per-user global config. Required for this legacy path when stdin is not a TTY. |
