...
| `INTEG` | The workspace's integration tier: `conventional` when the runner emits recognized markers (Claude Code, Codex), `degraded` when Shelbi can only poll it. |
| `STATE` | The live pane state (see below). |

```text
NAME         HOST     RUNNER         AGENT          INTEG          STATE
alpha        hub      opus           developer      conventional   in_progress: t-009
bravo        hub      opus           -              conventional   idle
charlie      hub      opus           review         conventional   review: t-004
delta        devbox   sonnet         qa             degraded       in_progress: t-017
echo         devbox   sonnet         -              conventional   idle
```
...
the poller's finer-grained read of an in-flight task (`working`,
`awaiting_input`, `blocked`), use [`status`](#status).

## set-runner

```text
shelbi workspace set-runner <RUNNER> [NAMES]...
...
| `[NAMES]...` | string list | — | Workspace names to update. |
