...
order: 22
summary: Inspect and control the project's declared workspace pool — list slots with their host, runner, and loaded agent, change slot runners, and stop stuck panes.
---
...
use it to observe (what's idle or working, which runner and agent are loaded
where), change existing slot runners, and intervene (stop a pane to release a
stuck task).
