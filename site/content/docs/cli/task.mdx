...
- [Columns](/docs/concepts/columns) — what each column means and how
  transitions trigger orchestrator behavior.
- [Workflows](/docs/concepts/workflows) — the schema behind `--workflow`
  and what `--to` is validated against on `task move`.
- [`shelbi workflow`](/docs/cli/workflow) — list, show, scaffold, and
  edit the workflow YAMLs tasks run under.
- [Workers](/docs/concepts/workers) — the pool model that `assign` /
  `start` route into.
