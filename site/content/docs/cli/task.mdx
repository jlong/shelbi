...
| `--workflow <NAME>` | string | — | Restrict to tasks pinned to the named workflow. Tasks with no explicit `workflow:` field are treated as the canonical `default` workflow. Composes with `--column` and `--ready`. |
...
Move a task between columns. Promoting a task to `todo` is the start signal
...
| Flag | Type | Default | Description |
| --- | --- | --- | --- |
| `--to <COLUMN>` | string | — | Destination column (required). Must be a status declared by the task's workflow. |
