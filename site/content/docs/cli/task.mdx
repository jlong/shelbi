...
title: shelbi task
order: 20
summary: Manage the project's Kanban task board — add, list, move, assign, start, and resume tasks from the CLI.
---
...
`resume` is the recovery counterpart to [`start`](#start). Where `start` wipes
the agent's context (kills the pane and re-checks-out a clean branch) — right
for a fresh dispatch, wrong for recovery — `resume` preserves the in-flight
state:
...
Recover a stalled worker without losing its in-flight work — relaunch the pane
and resume the conversation on the same task:
