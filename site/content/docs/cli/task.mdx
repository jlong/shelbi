...
a specific project (otherwise the active `$SHELBI_PROJECT` or the registered
project whose `work_dir` contains the current directory wins). It's omitted
from the per-subcommand tables below to keep them readable.
...
for the orchestrator — it picks a free workspace and calls `task start` on
its own.
