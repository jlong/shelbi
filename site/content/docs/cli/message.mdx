...
`ID`, `KIND`, and `BODY` are optional at the parser (so the `status` subcommand
parses) but required for the base `shelbi message` form — omitting any of them
...
| --- | --- | --- | --- |
| `[ID]` | string | — | Task id whose assigned workspace receives the message. Required for the base `message` form. |
| `[KIND]` | `reply` \| `directive` \| `context` | — | Message kind. Required for the base `message` form. |
| `[BODY]` | string | — | Message body. Required for the base `message` form. |
...
`msg_id` that `shelbi message` printed. Prints `delivered`, `queued`, or
