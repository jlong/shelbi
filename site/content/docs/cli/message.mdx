...
the file record and acknowledge its `msg_id`. Keeping this path file-based
avoids duplicate delivery and lets the message survive a pane restart.
...
| Delivery proof | Pane submission is verified; no worker semantic ack | Workspace tails and acknowledges the `msg_id` |
