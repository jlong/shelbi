...
Four columns, no `Review`. An issue branch is cut off `main` at
...
git:
  base_branch: main
  merge_strategy: squash               # one squashed commit per issue on trunk
...
An issue on the `InProgress → Done` edge may auto-merge when **all** hold:
...
Otherwise, hold the card in `InProgress` and surface it: "`<issue>` is
...
- [Feature-branch](/docs/guides/understanding-workflows/feature-branch) — the review-gated
  version of the same branch-per-issue shape.
