...
order: 33
summary: Branch per issue off main, open a PR, merge back. This is the default Shelbi shape — the workflow every project ships with, written out in full.
---

**Feature-branch** (a.k.a. GitHub-flow) is the simplest durable model:
`main` is always deployable, every issue cuts a short branch off it, opens
...
name: default
description: Feature-branch flow — branch per issue off main, PR, merge.
...
#
# transitions:
#   - { from: in-progress, to: review,   actions: [push_branch, open_pr] }   # push the issue branch, open a PR into main
