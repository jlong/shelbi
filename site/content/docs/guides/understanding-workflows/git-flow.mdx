...
workflow whose issues branch off and merge back into `develop`, and a
`release` workflow whose issue PRs `develop` into `main`. Hotfixes reuse
...
<AppMockup
  state={{
    ...
    readyReview: [
      {
        title: "Rework audit log schema",
        branch: "shelbi/rework-audit-log",
        location: "hub:3000",
      },
    ],
    queuedReview: [],
...
A release issue PRs the accumulated `develop` into `main`. Pre-fill its
...
- **Hotfixes** — titles prefixed `hotfix:` — use the `default` workflow
  ...
  merges to `main`, open a follow-up issue to merge `main` back into
...
follow-up issue rather than trying to encode it as a transition.
...
- [Workflows: the `branch:` task field](/docs/guides/getting-started/workflows#the-branch-task-field)
  — how a release issue operates on `develop` without cutting a new branch.
