...
someone else's fork, Shelbi doesn't cut branches or push here. Each issue
...
- **No `open_pr`.** The contributor already opened the PR from their fork.
  The issue just tracks it. Pre-fill `branch:` with the PR's head ref so
...
- [Workflows: the `branch:` task field](/docs/guides/getting-started/workflows#the-branch-task-field)
  — pointing an issue at an existing fork PR branch instead of cutting one.
