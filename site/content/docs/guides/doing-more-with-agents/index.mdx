...
The core loop is optimistic. The `developer` implements an issue and marks it
...
diff on the issue's branch, writes up what it finds with severity and
...
adversarial pass that every issue walks through before a person spends
...
[Agents](/docs/concepts/agents) concept describes: an issue is the work, a
...
- **Gate agents** review an issue and can bounce it back to an earlier status
...
- [Agents](/docs/concepts/agents) — the role/issue/workspace model, the
