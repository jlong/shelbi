...
agent is *actively working* the review while an issue sits here, it's
...
`owner: agent` makes the status the orchestrator's to act on, so issues
...
- **`adversarial-review → in-progress`** is the bounce-back. When the
  ...
  the issue drops to the developer with no git side-effects: same branch,
...
`push_branch` (push the issue branch to origin), `open_pr` (open its PR),
...
Review. Issues the developer marks ready flow into it, get auto-dispatched
