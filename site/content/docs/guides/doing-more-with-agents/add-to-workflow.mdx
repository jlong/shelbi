...
hub-side side-effects that fire on an edge; unlisted edges are pure status
changes. Route the developer's branch into adversarial review, then on to
human review, with a bounce-back for when the skeptic finds problems — and
keep `merge` on the single final edge into `done`:
...
- **`adversarial-review → in-progress`** is the bounce-back. When the
  skeptic files blockers, the task drops to the developer with no git
  side-effects — same branch, same PR, another pass.
