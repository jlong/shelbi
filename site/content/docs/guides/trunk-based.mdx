...
In Shelbi this is the [feature-branch model](/docs/guides/feature-branch)
with the `Review` handoff collapsed: the `merge` action moves onto the
`InProgress → Done` edge. Because
[Zen Mode's confidence bar is action-based](/docs/concepts/workflows#what-fires-zen-modes-high-confidence-bar)
...
Trunk-based reuses the shipped [status catalog](/docs/concepts/workflows#schema)
(`workflows/statuses.yml`) — it just leaves the `review` status out of the
flow. Identity (`name`, `category`) stays in the catalog; the workflow
references statuses by `id` and adds only `owner` and `agent:`:

```yaml
# workflows/trunk.yaml — reference-only (identity lives in statuses.yml)
name: trunk
...
[Zen judgment](/docs/getting-started/custom-workflow#2-tune-zens-judgment-categories-for-this-projects-reality)
...
- [Feature-branch](/docs/guides/feature-branch) — the review-gated
...
- [Workflows: what fires the high-confidence bar](/docs/concepts/workflows#what-fires-zen-modes-high-confidence-bar)
