...
`category` — lives once in `workflows/statuses.yml`, the project-wide
[status catalog](/docs/concepts/workflows#schema). Every workflow file
...
```yaml
# workflows/statuses.yml — the status catalog, shared by every workflow.
statuses:
...
[`target:`](/docs/concepts/workflows#per-transition-target). The
