...
order: 13
summary: Open the TUI, add an issue, watch a workspace pick it up, and review the diff.
---

<CopyPromptBanner prompt="Walk me through running my first Shelbi issue. Open the TUI with `shelbi`, add an issue to the backlog (through the orchestrator chat or `shelbi issue add`), promote it to todo so the orchestrator dispatches it to a free workspace, then review the resulting branch and merge it. Explain the five columns — Backlog → Todo → In Progress → Review → Done — and the workspace badges as we go." />

An issue moves through five columns: **Backlog → Todo → In Progress →
...
name, the nav (Chat, Issues, Activity), your declared workspaces, and any
issues waiting on review. The right pane is a real tmux pane: the
orchestrator agent by default, or one of the built-in views (Chat /
Issues / Activity) when you switch.
...
open a review, switch to the Issues board.
...
The orchestrator creates a markdown issue card in the **backlog**
...
```bash
shelbi issue add "Fix login bug on Safari"
```

Flags you'll reach for occasionally:

```bash
shelbi issue add "Fix login bug" \
  --id fix-login \
  --status todo \
...
- `--description` — fill in the issue body now instead of editing later.
...
Press `Ctrl+P` and pick "Issues", or hit `Enter` on the **Issues**
...
Adding an issue from the kanban view goes through the orchestrator, so it's
...
```bash
shelbi issue start fix-login-bug-on-safari --workspace delta
```
...
the issue prompt into the workspace's agent CLI. The sidebar badge next to
...
| `⏸` | paused — runner stalled on a usage/session limit |

A finished dev workspace doesn't get its own completion glyph: on promotion
the poller closes its session, so the row reads idle (`·`) immediately. The
"done, awaiting human" state lives in the review section, not on the
workspace row (that's where the `✓` you see below comes from).
...
into **Review**, and the workspace row drops back to idle (`·`). A `✓`
row appears in the sidebar's *Ready for Review* section:
...
Under the hood that's `shelbi merge fix-login-bug-on-safari`. If the review
flow already opened a PR for the branch, the CLI squash-merges that PR through
the GitHub CLI (so a PR-protected `default_branch` still accepts it); otherwise
it squash-merges locally into `default_branch`. Either way it prints the
resulting commit hash. Add `--pr` to open a fresh PR instead of merging.
...
```bash
shelbi issue move fix-login-bug-on-safari --to done
```
...
The orchestrator squash-merges on request but never marks an issue Done for
...
You added an issue, promoted it, watched a workspace pick it up, reviewed
...
volume: more issues at once, more workspaces, more machines.
