...
declared `workflow: task`. Fresh projects get `default_workflow: task`
written by the scaffold, so the review-gated `task` flow is the default out
of the box. If the project field is omitted (a pre-existing board that never
set it), Shelbi keeps the backward-compatible built-in fallback of `default`.
...
`main` as `task/<id>`, moves through `Backlog → Todo → In Progress →
...
click through the branch before you merge. The scaffold provisions one
`review`-tagged workspace slot on the hub for exactly this. See
...
The `default` workflow name still exists as a **built-in fallback** for
pre-existing boards. If a project has no `workflows/` directory at all
(legacy state), or a task names a `workflow:` that can't be found, Shelbi
falls back to the built-in `default`. Existing task files are not modified.
A task with no `workflow:` field is treated as belonging to the project
default workflow (`task` on a freshly scaffolded project), or `default` when
the project configures none, so there's nothing to backfill.
