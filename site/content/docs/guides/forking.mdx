...
carries the fork's PR branch in [`branch:`](/docs/concepts/workflows#the-branch-task-field)
...
<AppMockup
  state={{
    ...
    review: [{ title: "PR #401: perf: cache regex", workspace: "bravo" }],
  }}
/>
...
statuses:
  - { name: Incoming, category: backlog, owner: user                     }
  - { name: Triage,   category: ready,   owner: agent, agent: orchestrator }
  - { name: InReview, category: active,  owner: agent, agent: reviewer     }
  - { name: SignOff,  category: handoff, owner: user                      }
  - { name: Merged,   category: done,    owner: user                      }
  - { name: Declined, category: archived, owner: user                     }
...
Two side-effects that the [feature-branch](/docs/guides/feature-branch)
...
The `InReview` status names an [`agent: reviewer`](/docs/concepts/agents)
...
`SignOff` status stays `owner: user`: the gate is the whole point.
...
[trunk-based](/docs/guides/trunk-based): trunk-based leans *into*
...
- [Trunk-based](/docs/guides/trunk-based) — the opposite end of the
