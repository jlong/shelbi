...
[status catalog](/docs/concepts/workflows#schema) first —
`workflows/statuses.yml` — then reference them by `id` from the workflow:

```yaml
# workflows/statuses.yml — the forking project's status catalog.
statuses:
...
```yaml
# workflows/contribution.yaml — reference-only; identity lives in statuses.yml
name: contribution
