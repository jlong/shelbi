...
The shipped `default` workflow is a pure status track: six statuses and
**no** transitions, so every move is a plain status change. The scaffolded
`default.yaml` writes `transitions:` only as a commented optional block you
uncomment to add side-effects.
...
# transitions: omitted by default — every move is a pure status change.
# Uncomment to add hub-side side-effects on specific edges:
# transitions:
...
| `required_params` | list of string | no | — | Task frontmatter fields every task on this workflow must carry. Every `{{var}}` in [`git.base_branch`](#git) must appear here, or the workflow is rejected at load. See [Required params](#required-params). |
...
A workflow-scoped recipe for how the [Review agent](/docs/concepts/review-workspaces)
...
<Callout type="note" title="review: vs. a transition run: serve block">

The [`run` / `ready`](#run-ready-and-teardown) block on a review-entering
...
the contract that makes a templated [`git.base_branch`](#git) safe: because
...
| `branch` | string | no | project's `branch` | Full branch-name template for generated task branches in this workflow, rendered with `{{var}}` substitution when a task omits `branch:`. The shipped `task` workflow uses `branch: '{{github_user}}/{{id}}'`. Mutually exclusive with `branch_prefix`. |
| `branch_prefix` | string | no | project's `branch_prefix` | Shorthand alternative to `branch`: generates `<branch_prefix>/<task-id>` when a task omits `branch:`. Mutually exclusive with `branch` — declaring both in one `git:` block fails the load. |
...
Explicit task `branch:` frontmatter still wins over a workflow `branch`
