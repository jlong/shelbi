...
Worker hooks are deployed into each worktree under `.shelbi/hooks/`. Claude's
`.claude/settings.json` references `.shelbi/hooks/claude.*`; Codex receives
`core.hooksPath=.shelbi/hooks/codex.toml` on its launch command. Shelbi-owned
hook files stay out of user-owned `.codex/` configuration.
...
| `branch_prefix` | string | no | GitHub username, then `user` | Prefix for generated task branches when a task omits `branch:` and its workflow does not override the prefix. Shelbi cuts `<branch_prefix>/<task-id>`. |
...
`branch_prefix` accepts the same safe branch characters Shelbi allows for
generated refs: ASCII letters, numbers, `-`, `_`, and `/`. It must not end
with `/`. An explicit task `branch:` still wins over every prefix setting.
