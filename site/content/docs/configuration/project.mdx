...
| `workspace_permissions_mode` | string | no | — | Explicit project-wide permission ceiling rendered into the workspace settings template. Unset (the default) passes no `--permission-mode` and renders no `defaultMode`, so agents run under your own `~/.claude/settings.json` `permissions.defaultMode`. Set it to clamp every agent's requested mode downward. |
...
| `review` | [Review](#review) | no | all defaults | Review-interface behavior. Today just `diff_command`, the **View Diff** escape hatch for a diff tool that reviews a revision range instead of git's directory-diff pair. |
...
Behavior of the [review interface](/docs/concepts/review-workspaces#the-review-interface)'s
**View Diff** action.
...
The command runs in the review worktree with three placeholders substituted
(shell-escaped):
...
Leave `diff_command` unset and **View Diff** keeps the default `git difftool
-d -y` path, so existing setups are unchanged. Either way, a diff tool that
...
[config-upgrade pass](/docs/concepts/config-upgrade) materializes it for
...
Unlike the [`git.branch`](#git) template above, `pr-template.md` has **no**
...
The path is [config-mode](/docs/concepts/config-modes)-aware, resolving next to
the rest of the project's config half:
...
The default template tells the worker to write the body **for the human
reviewer**, grounded in the real problem the change solves rather than a copy of
the task requirements, in these sections, top to bottom:

1. **Summary**: a few sentences on the feature or fix and the problem it
   solves, so the reviewer understands the *why* before the *how*.
2. **Technical details**: key design decisions and rejected alternatives,
   tradeoffs, risky or subtle areas, and any migration or rollout notes.
3. **Data model changes**: a conditional section, included only when the change
   touches a database or schema, with a Mermaid ER or class diagram GitHub
   renders natively.
4. **UI changes**: a conditional section, included only when the change alters
   the interface, with a shareable screenshot when one exists or an ASCII
   wireframe otherwise.
5. **QA checklist**: concrete, checkable items a reviewer or tester can run
   through, using GitHub task-list syntax.

Conditional sections are omitted entirely when they do not apply, so a
non-schema, non-UI change carries only sections 1, 2, and 5.
...
- [Config upgrade](/docs/concepts/config-upgrade): the boot-time pass that
  materializes `pr-template.md` for older projects and heals drifted config.
