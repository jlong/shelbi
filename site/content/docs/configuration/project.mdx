...
| In-repo | The same flat `~/.shelbi/projects/<id>.yaml` carries every field; the committed `<repo>/.shelbi/project.yaml` holds only `name` (and an optional `display_name`) so teammates share the label. |
...
In both modes every field lives in the one flat
`~/.shelbi/projects/<id>.yaml` that `shelbi init` and `shelbi init --pick-up`
write. In-repo mode adds a committed `<repo>/.shelbi/project.yaml`, but that
file carries only `name` (plus an optional `display_name`) — the label
teammates share; every other field stays in your per-machine flat file, which
is where you edit `repo`, `machines`, `workspaces`, and the rest.

<Callout type="note" title="The two-file local.yaml split is migration-only">

Older projects (and any project touched by a `shelbi workspace` edit) may
carry a two-file split: **shared** fields (`name`, `default_branch`,
`default_workflow`, `orchestrator`, `agent_runners`, `zen`, …) in the
committed `project.yaml` and **user-local** fields (`repo`, `machines`,
`workspaces`, `editor`) in a per-machine
`~/.shelbi/projects/<id>/local.yaml`. Shelbi still reads that layout, but no
fresh `init` or `--pick-up` produces it, and a hand-created `local.yaml` is
shadowed by the flat file the loader prefers. The bucket lists that drive the
split — `SHARED_PROJECT_FIELDS` and `LOCAL_PROJECT_FIELDS` in
[`shelbi_core::model`](https://github.com/jlong/shelbi/blob/main/crates/shelbi-core/src/model.rs)
— govern only the migration and edit paths, not the flat file you get today.
