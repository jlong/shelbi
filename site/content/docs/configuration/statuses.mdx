...
<Callout type="note" title="Migrated from statuses.yml">

Older versions wrote this catalog as `statuses.yml`. Shelbi now
standardizes on `.yaml` and migrates automatically on load: a legacy
`statuses.yml` is renamed to `statuses.yaml` the next time the project
loads (only when no `statuses.yaml` already exists). No action needed.
