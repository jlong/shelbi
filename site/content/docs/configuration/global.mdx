...
| --- | --- | --- | --- | --- |
| `keymap.zen_toggle` | [chord](#zen_toggle-chords) | no | `alt-z` | Chord that toggles Zen Mode on and off. **Legacy** — a one-shot migration moves a non-default value into `keys.yaml` (`defaults.global.zen_toggle`) and resets this field. Prefer setting it in `keys.yaml`. |
...
<Callout type="note" title="Migrated from keys.yml">

Older versions named this file `keys.yml`. Shelbi now standardizes on
`.yaml` and migrates automatically on load: a legacy `keys.yml` is
renamed to `keys.yaml` on the next launch (only when no `keys.yaml`
