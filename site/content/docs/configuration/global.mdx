...
title: Global config
order: 33
summary: Field-by-field reference for the hub-wide files — ~/.shelbi/config.yaml (UI preferences) and ~/.shelbi/keys.yml (keybindings).
---
...
preferences) and `keys.yml` (keybindings). Both are optional — absent or
...
`$SHELBI_ROOT`, then the legacy `$SHELBI_HOME`, before falling back to
...
Per-user UI preferences. Distinct from `shelbi.yaml` (assistant
identity, wizard state) so a future `shelbi config reset` can wipe UI
tweaks without touching onboarding. A missing file — or one that omits a
block — resolves to defaults.

**Path:** `~/.shelbi/config.yaml`

| Field | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `keymap.zen_toggle` | [chord](#zen_toggle-chords) | no | `alt-z` | Chord that toggles Zen Mode on and off. **Legacy** — a one-shot migration moves a non-default value into `keys.yml` (`defaults.global.zen_toggle`) and resets this field. Prefer setting it in `keys.yml`. |
...
- **`shelbi.yaml`** — hub-wide bookkeeping: the assistant identity
  (`{{assistant_name}}`, substituted into agent prompts) and a
  per-project `last_launched` index. Written by the setup wizard.
