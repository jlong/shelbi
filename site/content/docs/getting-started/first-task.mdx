...
- `--column` — drop straight into todo (or any other column).
...
- Read the **Concepts** docs for the full mental model of workers,
  columns, the events log, and the orchestrator.
- Browse the **CLI reference** for every flag on every `shelbi`
  subcommand.
- Edit `~/.shelbi/projects/<name>.yaml` to grow your worker pool, then
  `shelbi reload` to pick up the change.
