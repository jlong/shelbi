...
nothing is loaded into a slot yet. If a pane is dead, jump to the
workspace (`Enter` on its sidebar row) — the session may have stalled on
a permission dialog. Resolving it puts the workspace back in the
rotation.
...
pinned to, and a busy row shows the agent it's running:

```
 — hub —
 ⏵ alpha    developer   add-csv-export-to-reports
 ⏵ bravo    developer   fix-cookie-domain-bug-on-safari-sso
 ⏵ charlie  developer   tighten-rate-limit-error-copy
