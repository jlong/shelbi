...
- An **agent** is a system prompt + skill set — the `developer`, `qa`, or
  `security-review` role. It's loaded *into* a workspace when a task is
  dispatched.
...
<Callout type="note" title="Legacy naming: worker">

Earlier versions of Shelbi called this slot a *worker* and conflated it
with the agent running inside. The CLI keeps `shelbi worker` as a
[deprecated alias](/docs/cli/workspace#deprecation-shelbi-worker), and the
events log still uses the `worker=<name>` token for backward compatibility.
...
A slot can also be marked `role: review` — a scarce
[review workspace](/docs/concepts/review-workspaces) that loads a finished
branch and boots its dev server for a human to click through, rather than
picking up development work. Everything on this page applies to it too; the
role just changes what gets dispatched there.
...
2. The worktree is switched to the task's branch — creating the branch off
   `default_branch` if it doesn't exist, refusing to switch if there are
   uncommitted changes.
...
clean context — no leakage between tasks on the same workspace.
...
2. Moves the task to the next `handoff` status (`review` in the default
   workflow).
3. Clears the marker.
4. Appends `task=<id> in_progress -> review reason=worker:review-marker` to
   the events log.
...
- [Review workspaces](/docs/concepts/review-workspaces) — the `role: review`
  variant that loads and *serves* a finished branch for human review.
