...
title: Agents
order: 11
summary: An agent is a role, a system prompt plus a skill set, not the issue it's handed or the slot it runs in. Shelbi ships six (orchestrator, developer, review, and the qa, security, and adversarial reviewers); each is a directory you can edit, and you can author more.
---
...
*not* the issue it's handed, and *not* the slot it runs in. Those are separate
things:
...
on one issue and a reviewer agent on the next, and what lets a
...
| Agent | Role |
| --- | --- |
| `orchestrator` | The agent you talk to. Turns requests into cards, dispatches issues to free workspaces, tails the events log, reports back. Runs in window 1, never in a workspace. See [Orchestrator](/docs/concepts/orchestrator). |
...
You rarely assign an agent to an issue directly. Instead, a
