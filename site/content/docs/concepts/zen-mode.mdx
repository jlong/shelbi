...
`zenmode.md`; it only migrates exact legacy stock command snippets and
leaves your surrounding prose intact.
...
Earlier versions inlined the judgment categories and merge conditions in
the orchestrator's prompt, the `## Zen Mode` section of
`agents/orchestrator/instructions.md`. They no longer live there. That
section now just tells the orchestrator to treat `zenmode.md` as the
source of truth and to re-read it on the heartbeat cues above. So
