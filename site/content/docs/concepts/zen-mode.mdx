...
```markdown
Zen: auto-promote eligible backlog, review each handoff yourself (diff vs the issue's acceptance criteria + scope), and run the exact-provenance merge flow without asking.
...
6. Freeze the probe's `repository`, `repository_id`, `base_branch`, `base_sha`,
   ...
   is idempotent). Exit code 1 is a real mismatch: leave the issue in `handoff`
   and re-probe.
...
8. If green, run `shelbi zen pr-merge <pr-number> --match-repository <repository> --match-repository-id <repository_id> --match-base-branch <base_branch> --match-base-commit <base_sha> --match-integration-commit <integration_sha> --match-head-commit <head_sha>`
   ...
   leave the issue in `handoff` and follow the command's human-review action.
   Only after a candidate SHA result, move the issue into `done` and emit
