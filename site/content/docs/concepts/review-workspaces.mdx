...
(set-AND). Empty required tags — the default — match any free workspace, which
is the historical "any idle slot" behavior. So `tags: [review]` on the status
...
Earlier releases modeled review with bespoke keys instead of the generic
primitives above. They still load, so an old project YAML keeps working, but
they're deprecated:

- **`role: review`** on a workspace (and `role: dev`) — folded into `tags` on
  load: `role: review` becomes the `review` tag, `role: dev` becomes no tag.
  The key is never written back; prefer `tags:` directly.
- **The `review:` block** on the project (`base_port`, `port_stride`, `setup`,
  `serve`, `ready_probe`, …) — no longer a field. It's silently ignored on
  load. Its job is now done by workspace `slot` / `$SLOT` and transition
  `run` / `ready` commands.
