...
- The leading `project=<name>` scope is load-bearing, exactly as for
  workspace lines.
- `workflow=<name>` is the workflow the task runs under (from its
...
- The line always carries `zen_eligible=` (how many tasks Zen could act
...
- Adaptive cadence: the poller holds at the standard `interval`
  ...
  for a silently-stuck task about every five minutes. Backing off on "no
