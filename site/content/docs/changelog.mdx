...
machines over SSH) carries a per-task message log, and Claude Code workers inject
those messages via `SessionStart` + `Stop` hooks; non-hook runners poll for them
in the prompt. On top of it sits a clarification loop (a worker can ask a
