...
**Hub heartbeats.** The hub-side worker-state poller now appends a periodic
...
don't see padding. Heartbeats are filtered out of the TUI activity feed by
default — they'd produce one "nothing happened" row every few minutes —
but show up verbatim under `shelbi events tail`. See
