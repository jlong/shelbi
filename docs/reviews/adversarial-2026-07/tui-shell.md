...
- crates/shelbi-tui/src/zen_probe.rs (406 lines)
...
- **Poller thread vs UI thread**: no shared in-memory state — the poller and the
  ...
  poll threads (F7).
- **zen_probe.rs subprocess handling**: the seed's premise doesn't hold —
...
- **Recommendation:** at minimum, re-check dirtiness immediately before the
  rebase inside a single remote invocation (`git status --porcelain && git
  rebase …` under one shell), and document that post-marker work in the worktree
...
- **Where:** crates/shelbi-tui/src/poller.rs:378 (`save_workspace_status`), 382-388 (`append_workspace_event`); crates/shelbi-tui/src/app.rs:708 (`load_workspace_status`); crates/shelbi-state/src/workspace_status.rs:131-134 (path, for context)
- **Category:** assumption
- **Severity / Confidence:** medium / certain
...
- **Recommendation:** saturate instead of `as` (`.min(u16::MAX as usize) as
  u16`) as a stopgap; properly fixed by building only the viewport window
  (F2).
- **Effort:** S
...
- **Where / Evidence / Recommendations:**
  ...
  5. **Zen chooser has no escape/timeout for keyless sessions** —
     zen_probe.rs:203-222: on a first run in an environment that delivers no
     ...
     bound it. Also, the probe overlay (zen_probe.rs:194) is drawn once and not
