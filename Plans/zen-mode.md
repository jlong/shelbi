...
- **Merge conditions are also prompt-driven.** Symmetric to auto-promotion: Rust supplies primitives (`shelbi zen probe`, `pr-create`, `ci-watch`, `pr-merge`) that report structured data and perform single-purpose actions. The orchestrator prompt's `## Zen Mode: Merge Conditions` section encodes the policy — which probe results gate the merge, what the diff-size thresholds are, what counts as a CI-pass, etc. Users edit the prompt section per project to tighten/loosen conditions, add new pre-merge probes, or accept partial CI failures.
