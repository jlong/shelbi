...
- **Per-worker preferred agent?** Should a worker slot be able to declare `prefers_agent: developer` so the orchestrator routes that worker to developer-statuses when possible? Useful if certain hosts have tools or auth only Developer needs. Probably overkill for v1 — defer.
