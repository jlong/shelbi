...
- While the budget was at zero every board operation failed: `issue list/show/start/ resume/move`, `zen probe`, `zen pr-create`, `orchestrator events next`, dispatch, and the
...
drain, `zen scan`) use one query with aliases (`a: issue(number: 1) {...} b: issue(number: 2) {...}`), still one point each and one round trip.

Unknown id (not in the open index): `search(query: "repo:o/r label:\"shelbi:id/<label>\"", type: ISSUE, first: 2)` — one point — replaces today's label-filtered REST list.
