...
When the orchestrator dispatches a task to a workspace, it now resolves three things:

1. **Which workspace?** Same logic as today — first free workspace in declaration order, honoring `prefers_machine`. (Section title also renames in v1: §5 is "Workspace dispatch with agents" once the CLI rename lands.)
...
- Passes `--system-prompt $(cat agents/<agent>/instructions.md)` (or equivalent), prepended with `agents/_shared/preamble.md` if the project has one.
...
- Skips `CLAUDE.md` entirely — it's deprecated in v1. Existing `CLAUDE.md` files trigger a one-time migration hint pointing at `agents/_shared/preamble.md` and `agents/orchestrator/instructions.md`; the read path is removed in v2.
