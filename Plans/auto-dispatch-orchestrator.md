# Auto-dispatch: orchestrator-driven, fed by the events log
...
- **Initial task snapshot** — `shelbi task list` (already exists). Gives all columns + priorities + assigned_to.
...
7. Manually move a task backlog → in_progress (skip todo) via the TUI. The orchestrator should ignore it — manual-override path stays open.
