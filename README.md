...
On task dispatch, Shelbi writes its runner hook implementations under the
worktree's `.shelbi/hooks/`. Claude's `.claude/settings.json` is only a thin
adapter that points at `.shelbi/hooks/claude.*`; Codex is launched with
`core.hooksPath=.shelbi/hooks/codex.toml`. Shelbi does not create or overwrite
