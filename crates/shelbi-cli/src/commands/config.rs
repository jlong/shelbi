...
pub enum ConfigCmd {
    ...
    /// `~/.shelbi/keys.yaml` as a starting point for customization.
    DumpKeybindings {
        ...
        /// `keys.yaml`) is refused so a stray `-o` can't silently erase
        /// your customizations.
        #[arg(long)]
        force: bool,
    },
    /// Validate `~/.shelbi/keys.yaml` and print any errors / warnings.
...
# This is the full default keymap. Drop into ~/.shelbi/keys.yaml and edit.
...
fn dump_keybindings(out: Option<PathBuf>, force: bool) -> Result<()> {
    ...
    match out {
        Some(path) => {
            // Never silently clobber an existing file: `-o ~/.shelbi/keys.yaml`
...
/// the keys.yaml path with the diagnostic's logical location appended.
fn format_diag_line(
    keys_path: &str,
    config_path: &str,
    level: &str,
...
/// Tilde-shorten an absolute path for display: `/Users/foo/.shelbi/keys.yaml`
/// → `~/.shelbi/keys.yaml`. Falls back to the raw display form if no home
