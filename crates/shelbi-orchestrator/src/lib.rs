...
/// Shell snippet that clamps the traveling sidebar pane's width to
/// `SIDEBAR_TARGET_PCT%` of its current window, bounded to `[MIN, MAX]`.
/// Resolves the pane from the `SHELBI_SIDEBAR` session env so it clamps
/// the sidebar wherever it currently lives (it travels between windows),
/// not a fixed `dashboard.{left}` position. Written to disk so the hook
/// can invoke it by path without inlining shell into a tmux command-list
/// string.
fn sidebar_clamp_script(session: &str) -> String {
    format!(
        "#!/bin/sh\n\
         ...
         [ -z \"$sb\" ] && exit 0\n\
         w=$(tmux display-message -p -t \"$sb\" '#{{window_width}}' 2>/dev/null)\n\
         [ -z \"$w\" ] && exit 0\n\
         c=$((w * {pct} / 100))\n\
         [ \"$c\" -lt {min} ] && c={min}\n\
         [ \"$c\" -gt {max} ] && c={max}\n\
         tmux resize-pane -t \"$sb\" -x \"$c\" 2>/dev/null || true\n",
        sess = session,
        env = SIDEBAR_ENV_KEY,
...
///
/// Width preservation: the pane's live `#{pane_width}` is captured *before*
/// the move and reapplied with `join-pane -l`, so a width the user set by
/// hand (a manual drag) is what follows the sidebar thereafter — a switch
/// never reflows it to a default split size.
fn sidebar_travel_script(session: &str) -> String {
    format!(
        "#!/bin/sh\n\
         # Auto-generated by shelbi; rewritten on every `ensure_dashboard`.\n\
         sess='{sess}'\n\
...
fn ensure_sidebar_pin(host: &shelbi_core::Host, session: &str) {
    ...
    if let Ok(pane_id) = shelbi_ssh::run_capture(
        ...
        if !pane_id.is_empty() {
            let _ = set_session_env(host, session, SIDEBAR_ENV_KEY, pane_id);
        }
    }
}
...
fn ensure_sidebar_travel(
    ...
    std::fs::write(&travel_script_path, sidebar_travel_script(session)).map_err(Error::Io)?;
...
fn reload_sidebar(session: &str, project_name: &str, shelbi_bin: &str) -> PaneReloadStatus {
    let sidebar_target = read_session_env_var(&Host::Local, session, SIDEBAR_ENV_KEY)
        .ok()
        .flatten()
        .filter(|id| pane_id_alive(&Host::Local, id).unwrap_or(false))
        .unwrap_or_else(|| format!("{session}:dashboard.{{left}}"));
...
mod pane_cmd_tests {
    ...
    fn sidebar_travel_script_joins_preserving_width_and_avoids_focus_steal() {
        let out = sidebar_travel_script("shelbi-myapp");
    ...
    fn sidebar_scripts_are_syntactically_valid_shell() {
        ...
        for (label, script) in [
            ...
            ("travel", sidebar_travel_script("shelbi-myapp")),
        ] {
