...
pub fn start_worker_on_task(spec: StartSpec<'_>) -> Result<TmuxAddr> {
    ...
    //    `LANG=C.UTF-8` is a cheap, low-risk guard: a non-interactive SSH
    //    launch can leave the tmux server in the C locale, and while claude
    //    happens to emit its `❯` glyph anyway, forcing UTF-8 keeps every
    //    box-drawing/glyph path well-defined regardless of host config.
